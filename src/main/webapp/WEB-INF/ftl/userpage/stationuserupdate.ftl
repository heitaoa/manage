		...
		<form class="form-horizontal" action="#">
			...
			<div class="form-body">
				<div class="form-group">
					...
					<div class="col-md-4">
						<input type="text" class="form-control input-medium input-inline" readOnly="true" id="userinfo" value="${user.userid!} ${user.user_name!}"><a class="btn blue" data-toggle="modal" href="#show1">添加</a>
				...
				<div class="form-group">
					...
					<div class="col-md-4">
						...
						<div style="width:100px; float:left;"><input type="checkbox"  class="checkboxes" value="2" name="level" <#if user.ulevel?index_of("2")!=-1>checked="checked"</#if>>销售</div>
						<div style="width:100px; float:left;"><input type="checkbox"  class="checkboxes" value="3" name="level" <#if user.ulevel?index_of("3")!=-1>checked="checked"</#if>>客服</div>
						<div class="chearfix"></div>
						<div style="width:100px; float:left;"><input type="checkbox"  class="checkboxes" value="5" name="level" <#if user.ulevel?index_of("5")!=-1>checked="checked"</#if>>分站编辑</div>
						</div>
					</div>
				</div>
				...
				<div class="form-group" id="diyu" style="display:none;" >
					...
					<div class="col-md-4">
						<div class="input-group">
						<select class="form-control input-small input-sm input-inline"  id="prov" onfocus="this.defOpt=this.selectedIndex" onchange="this.selectedIndex=this.defOpt;">
				...
				<div class="form-group" id="pingpai3" style="display:none;" >
					<label class="col-md-3 control-label">未分配品牌:</label>
					<div class="col-md-4">
						<div class="input-group">
						<select class="form-control input-medium input-sm "  id="barnd3">
						<#list unUsedbrands as brand>
						<option value="${brand.iway!}">${brand.wayname!}&nbsp;${brand.iway!}</option>
						</#list>
						</select>
						</div>
					</div>
			...
			<div class="form-actions">
				<div class="row">
					<div class="col-md-offset-3 col-md-9">
						<button class="btn blue" type="button" onClick="saveUser();">修改</button>
					</div>
				</div>
			</div>
		</form>
...
<div id="show1" class="modal fade in" tabindex="-1" data-width="760" aria-hidden="false" style="display: block; width: 760px; margin-left: -379px; margin-top: -148px;">
	...
	<div class="modal-body">
		...
		分站:<select class="form-control input-small input-sm input-inline" id="fsite"  onfocus="this.defOpt=this.selectedIndex" onchange="this.selectedIndex=this.defOpt;">
			<#list siteList as site>
			<option value="${site.id!}" <#if site.id = userInfo.sub_sites_id2>selected='true'</#if> >${site.name!}</option>
			</#list>
			</select>
...
function createTree(treeMenu){
	...
	var id = $("#id").val();
	$.getJSON("initCusULevelTree?id="+id,function(data){
...
var levelx = "${user.ulevel!}";
if(levelx.indexOf("1")!=-1||levelx.indexOf("2")!=-1||levelx.indexOf("3")!=-1||levelx.indexOf("6")!=-1){
    	$("#diyu").css({display:"block"});
    	$("#pingpai1").css({display:"block"});
    	$("#pingpai2").css({display:"block"});
    	$("#pingpai3").css({display:"block"});
    }else{
...
$.ajax({
    		...
    		success : function(data){
				...
				{
					var html ='';
					if(city[i].catalogid==${user.prov!})
						html ='<option value='+city[i].catalogid+' selected="true">'+city[i].lname+'&nbsp;'+city[i].catalogname+'</option>';
					else
...
$.ajax({
    		url : 'getCityInfo?city='+${user.prov!},
    		dataType : 'jsonp',
    		jsonp:'jsonpcallback',
    		success : function(data){
	    		city = data;
...
function saveUser(){
	...
	$.getJSON("updateUser", { userid:userid,level: encodeURI(value),prov: prov,city:city,brands:encodeURI(array),id:id,detail:detail}, function(data){
		if(data>0){
			alert("修改成功!");
