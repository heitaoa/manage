...
<div class="portlet box blue">
						...
						<div class="portlet-body form">
							...
							<input type="hidden" name="carGroupbuy.adddate" value="${carGroup.adddate}">
							<input type="hidden" name="carGroupbuy.id" value="${carGroup.id}">
							<input type="hidden" name="carGroupbuy.adddate" value="${carGroup.adddate}">
							<input type="hidden" name="carGroupbuy.sites_id" value="${stationId!}"   id="stationId">
							<input type="hidden" name="page" value="${page!}">
							<input type="hidden" name="type" value="${type!}">
							<input type="hidden" name="keywords" value="${keywords!}">
							<input type="hidden" name="stationId" value="${stationId!}">
							<input type="hidden" name="isdelete" value="${isdelete!}">
							<input type="hidden" name="pinpai" value="${pinpai!}">
							<table style="margin-left:15px;" class="xwbg1 customer_info_table">
							
							<tr>
							<td><span>活动种别:</span></td>
							<td colspan=3>
							<select class="form-control input-medium input-sm　" name="carGroupbuy.buy_type" id="buy_type">
                          ...
                          <option value="1">不向前台展示</option>
							...
							<td>
							<select class="form-control input-medium input-sm　"  id="pinpai">
							...
							<td colspan=3>
							<input type="text" class="form-control input-inline input-medium" name="carGroupbuy.car_info" value="${carGroup.car_info!}" id="car_info"> <em>有则直接显示</em> 
							</td>
							</tr>
							...
							<td><span>标题:</span></td>
							<td colspan=3>
							<input type="text" class="form-control input-inline input-medium" name="carGroupbuy.title" id="title" value="${carGroup.title}"> 
							</td>
							</tr>
							...
							<div  data-date-format="yyyy-mm-dd" class="input-group  date date-picker input-inline">
								<input type="text"  class="form-control input-medium input-inline" name="carGroupbuy.begdate" readOnly="true" id="begdate"  value="${carGroup.begdate}">
							...
							<td colspan=3>
							<div  data-date-format="yyyy-mm-dd" class="input-group  date date-picker input-inline">
								<input type="text"  class="form-control input-medium input-inline" name="carGroupbuy.enddate" readOnly="true" id="enddate"  value="${carGroup.enddate}">
								<span class="input-group-btn input-inline">
								<button type="button" class="btn default input-inline"><i class="fa fa-calendar input-inline"></i></button>
								</span>
							</div>
							...
							<td colspan=3>
							<input type="text" class="form-control input-inline input-medium" name="carGroupbuy.time_info" value="${carGroup.time_info!}" id="time_info"> <em>有则直接显示</em>
							</td>
							</tr>
							...
							<td colspan=3>
							<input type="text" class="form-control input-inline input-medium" name="carGroupbuy.address" id="address" value="${carGroup.address}">
							</td>
							</tr>
							...
							<td colspan=3>
							<input type="text" class="form-control input-inline input-medium" name="carGroupbuy.info" id="info"  value="${carGroup.info}">
							</td>
							</tr>
							...
							<td colspan=3>
							<input type="text" class="form-control input-inline input-medium" name="carGroupbuy.register_num" id="num" value="${carGroup.register_num}">
							</td>
							</tr>
							...
							<td colspan=3>
							<input type="text" class="form-control input-inline input-medium" name="carGroupbuy.surplus_num" id="surplus_num" value="${carGroup.surplus_num}">
							</td>
							</tr>
							...
							<td><span>售价:</span></td>
							<td colspan=3>
							<input type="text" class="form-control input-inline input-medium" name="carGroupbuy.price" id="price" value="${carGroup.price}"><em>单位:万</em>
							</td>
							</tr>
							...
							<td colspan=3>
							<select class="form-control input-small input-sm　" name="carGroupbuy.active_type" id="active_type">
							...
							<td colspan=3>
							<img src="" id="img"><br>
							<input type="text" class="form-control input-inline input-medium"  name="carGroupbuy.pic"  readOnly="true" id="pic" value="${carGroup.pic}">
							...
							<button class="btn blue" type="button"  id="add"><i class="fa fa-check"></i>修改</button>
...
var active = document.getElementById("active_type");
for(var i=0;i<active.options.length;i++) {  
            if(active.options[i].value == ${carGroup.active_type!}) {  
                active.options[i].selected = true;  
                break;  
            } }
 var buy = document.getElementById("buy_type");
for(var i=0;i<buy.options.length;i++) {  
            if(buy.options[i].value == ${carGroup.buy_type!}) {  
                buy.options[i].selected = true;  
                break;  
            } }
 var isdelete = document.getElementById("isdelete");
for(var i=0;i<isdelete.options.length;i++) {  
            if(isdelete.options[i].value == ${carGroup.isdelete!}) {  
                isdelete.options[i].selected = true;  
                break;  
            } }
//$("#active_type").find("option").eq(${carGroup.active_type!}).attr("selected",true);
//$("#buy_type").find("option").eq(${carGroup.buy_type!}).attr("selected",true);
$.ajax({
    		url : 'getCarBrand',
    		dataType : 'jsonp',
    		jsonp:'jsonpcallback',
    		success : function(data){
				...
				{
					var html;
					if(carBrand[i].iway=="${carGroup.iway!}")
...
$.ajax({
    		url : 'getCarsx',
    		dataType : 'jsonp',
    		data :{brandNmae:encodeURI("${carGroup.iway!}")},
    		jsonp:'jsonpcallback',
    		success : function(data){
	    		carBrand = data;
				...
				{
					var html;
					if(carBrand[i].catalogid==${carGroup.fatherid!})
					 html ='<option value='+carBrand[i].catalogid+' selected="true" >'+carBrand[i].lname+'&nbsp;'+carBrand[i].catalogname+'</option>';
					 else
		...
		$.ajax({
    		url : 'getCars?fatherid='+${carGroup.fatherid!},
    		dataType : 'jsonp',
    		jsonp:'jsonpcallback',
    		success : function(data){
	    		carBrand = data;
	    		$("#kuanshi").append("<option value='-1'>请选择款式</option>");
				for(var i =0 ;i<carBrand.length;i++)
				{
					var html;
					if(carBrand[i].catalogid==${carGroup.carid!})
					 html ='<option value='+carBrand[i].catalogid+' selected="true" >'+carBrand[i].lname+'&nbsp;'+carBrand[i].catalogname+'</option>';
					 else
					 html ='<option value='+carBrand[i].catalogid+'>'+carBrand[i].lname+'&nbsp;'+carBrand[i].catalogname+'</option>';
					$("#kuanshi").append(html);
...
$("#pinpai").change(function(){	
	...
	document.forms[0].action = "updateSaleFavored";
