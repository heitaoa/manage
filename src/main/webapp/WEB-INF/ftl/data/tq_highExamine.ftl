...
<div class="row">
					...
					<table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample_1_info">
						...
						<tbody>
							...
							<tr>
								...
								<!-- 是否同意扣除金额 -->
								<td tag="is_agree_deduct" val="${list.get("is_agree_deduct")}">
	...
	<div class="col-md-12">
		<div id="modal_tongguo" class="modal fade" tabindex="-1" data-backdrop="static" data-width="600">
	        <div class="modal-header">
	            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
	            <h4 class="modal-title">是否抵扣执行金额</h4>
	        </div>
	        <div class="modal-body">
	        	<form id="form_tongguo" class="form-horizontal" action="./promo_per_pro_function_sub" method="post"  role="form">
	            	<div class="form-body">
	            		<div class="row">
	            			<div class="col-md-12">
	            				<div class="form-group">
				                    ...
				                    <label class="col-md-5 control-label" style="text-align: left;" id="modal_tiqu_num_1"></label>
				                </div>
				                <div class="form-group">
				                	<label class="col-md-5 control-label">是否抵扣执行金额：</label>
				                    <div class="col-md-5 radio-list">
				                        <label class="radio-inline">
										<input type="radio" name="is_deduct" id="optionsRadios4" value="1" checked>是</label>
										<label class="radio-inline">
										<input type="radio" name="is_deduct" id="optionsRadios5" value="0">否</label>
				                    </div>
				                </div>
	            			</div>
	            		</div>
	        ...
	        <div class="modal-footer">
	            ...
	            <button id="bt_tongguo_sub" type="button" tag="" class="btn blue">提交</button>
	        </div>
	    </div>
	</div>
	            		...
	            		<div class="row">
	            			<div class="col-md-12">
	            				<div class="form-group">
				                	...
				                	<input type="hidden" name="tiqu_num" value="" />
				                    <label class="col-md-8 control-label" style="text-align: left;" id="modal_tiqu_num_2"></label>
				                </div>
				                <div class="form-group">
...
<script type="text/javascript">
	$(function(){
		$(".btn_yes").click(function(){
			var isagree = $(this).parents("tr").find("td[tag='is_agree_deduct']").attr("val");
			if(isagree=="1"){
				$("#modal_tiqu_num_1").html($(this).attr("goal")).prev().val($(this).attr("goal"));
				$("#modal_tongguo").modal();
				return;
			}
			
			$.post("./tiqu_high_examine_allow",{tiqu_num:$(this).attr("goal")},function(data){
				if(data==1){
					alert("审核已经通过！");
		...
		$(".btn_no").click(function(){
			$("#modal_tiqu_num_2").html($(this).attr("goal")).prev().val($(this).attr("goal"));
			$("#modal_bohui").modal();
		});
		
		$("#bt_tongguo_sub").click(function(){
			$.ajax({
			    type: "POST",
			    url: "./tiqu_high_examine_allow",
			    data: $('#form_tongguo').serialize(),
